...
const otpAuth = asyncHandler(async (req, res, next) => {
    ...
    const redisKey = email + Date.now().toString();
    await redisClient.set(redisKey, encoded);

    const token = jwt.sign({ otpKey: redisKey, email }, process.env.JWT_KEY, { expiresIn: "5m" });
    if (req.cookies) {
...
const otpVerifyV2 = asyncHandler(async (req, res, next) => {
    ...
    const otpData = await redisClient.get(tokenData.otpKey);
    console.log("Token Data printiing form redis get:", otpData);

    if (!otpData) return genRes(res, 500, false, "OTP Information not found on Server!");

    const status = await bcrypt.compare(String(otp), otpData);
    if (status === true) {
