...
export const checkCookie = asyncHandler(async (req, res, next) => {
    ...
    if (!token || !verifyTokens(data?.userId, token, "userToken")) {
...
export const checkRefreshCookie = asyncHandler(async (req, res, next) => {
    ...
    if (!token || !verifyTokens(data?.userId, token, "refreshToken")) {
        return res.status(401).json({ success: false, message: "User session expired or not found,Please Login !" });
    };
...
export const checkValidity = async (req, res) => {
    ...
    // )
    if (!token || !refreshToken) {
        return res.status(401).json({ success: false, message: "No session Information found on Request,Please Login!" });
    };
